...
fn a_polymorphic_definition_crosses_a_becki_boundary_and_is_fresh_at_every_call() {
    // docs/27 §27.10 named the shape of this gap for recursive types: "no *project* test imports a
    // module whose published type is recursive, so separate compilation over recursive types is
    // compiled-and-believed rather than measured". User-written polymorphism (docs/27) would have
