...
pub fn apko(graph: &InfraGraph) -> String {
    let account = crate::NONROOT;
    let command = crate::command();
    format!(
        "# Generated by `beck build` from {app}. Do not edit.\n\
         #\n\
         # `beck image` builds this image in-process and needs none of the commands below\n\
         # (docs/06 §6.2, docs/96). They are here because this config is also the *description* of\n\
         # the image — what a reader checks, and what somebody who would rather use apko runs:\n\
         #\n\
         ...
         contents:\n  repositories:\n    - {repository}\n    \
         - '@local ./packages'\n  keyring:\n    \
         - {repository}/wolfi-signing.rsa.pub\n    - ./local.rsa.pub\n  \
         packages:\n{packages}\n\
         entrypoint:\n  command: {entrypoint}\n\n\
         cmd: {args}\n\n\
        ...
        app = graph.app,
        repository = crate::sbom::REPOSITORY,
        ...
        packages = crate::sbom::packages(graph)
            ...
            .collect::<Vec<_>>()
            .join("\n"),
        // The same two arrays the OCI config carries, spelled apko's way: a config that described
        // one process and an image that ran another would be two artefacts nobody could compare.
...
pub fn melange(graph: &InfraGraph) -> String {
    format!(
        "# Generated by `beck build` from {app}. Do not edit.\n\
         ...
         pipeline:\n  - runs: |\n{installs}\n",
        app = graph.app,
        // Rendered from `crate::INSTALLS`, which is also what `beck image` writes into the layer:
        // two routes to one image that installed the same file at two paths would be a difference
        // nobody could see by reading either file (`crate::Installed`).
        installs = crate::INSTALLS
            .iter()
            .flat_map(|i| {
                let dir = i.path.rsplit_once('/').map(|(d, _)| d).unwrap_or("");
                [
                    format!("      mkdir -p \"${{{{targets.destdir}}}}{dir}\""),
                    format!(
                        "      install -m{mode:o} {from} \"${{{{targets.destdir}}}}{path}\"",
                        mode = i.mode,
                        from = i.from,
                        path = i.path
                    ),
