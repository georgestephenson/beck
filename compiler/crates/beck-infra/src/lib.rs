...
/// Where the toolchain lives inside the image. `beck run` is the image's whole entrypoint.
pub const TOOLCHAIN: &str = "/usr/bin/beck";

/// The account the image runs as.
///
/// One value, because four artefacts have to agree about it and three of them are generated by
/// different code: the apko config's `accounts:` block, the `/etc/passwd` and `/etc/group` the
/// in-process build writes ([`oci`]), the OCI config's `User`, and the pod's `securityContext`. A
/// mismatch between the first two and the last is the classic "works locally,
/// `CrashLoopBackOff` in the cluster", and it is invisible in review because each file is right on
/// its own.
...
/// The account every generated image runs as: non-root, and the same number everywhere.
pub const NONROOT: Account = Account {
    user: "nonroot",
    uid: 65532,
    group: "nonroot",
    gid: 65532,
...
/// A file the image contains that no package supplies.
///
/// Two of them — the toolchain and the program — and the reason this is a list rather than two
/// constants is that both routes to an image have to install them at the same paths with the same
...
pub struct Installed {
    ...
    pub path: &'static str,
    /// The permission bits it gets there.
    pub mode: u32,
    /// The file it is built from, relative to the build directory — the name melange's pipeline
    /// installs from and the name `beck image` looks for.
    pub from: &'static str,
}

/// The two files the application's own package installs, in install order.
pub const INSTALLS: [Installed; 2] = [
    Installed {
        path: TOOLCHAIN,
        mode: 0o755,
        from: "beck",
...
/// What the image runs, as `execve` would take it: the entrypoint, then its arguments.
///
/// The apko config spells this as `entrypoint.command` plus a `cmd:` string and an OCI config
/// spells it as two JSON arrays; both are rendered from here, so the image `beck build` describes
/// and the image `beck image` writes cannot start different processes.
pub fn command() -> Vec<String> {
    vec![
        ...
        "--store".to_string(),
        substrate::DEFAULT.store.to_string(),
        "--addr".to_string(),
