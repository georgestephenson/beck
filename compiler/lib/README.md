...
| A combining form the view engine has to **see** | a primitive, as above | `sort_by`, `filter_list`, `map_list`, `concat_lists`, `list_len`, `list_min`, `list_max`, `list_sum`, `list_unique`. Every one of these is expressible in Beck and none of them is a host's table — a sort is a sort, a minimum is a fold, and `list_unique` is the fold `unique` in [`collections.beck`](collections.beck) used to be, which is why that function's body is now a call to it. They are primitives because [`docs/23`](../../docs/23-incremental-views-report.md)'s engine maintains **what it can recognise**, and an aggregate written as a fold is one opaque operator recomputed in full on every event. This row is what the two above never explained: `sort_by` has always been a primitive and has never been a host's grammar |
...
| [`json.beck`](json.beck) | The other direction, and **the first library file to ship a macro**: `ToJson` with the base cases written by hand, and `derive_json`, which reads a `model`'s fields out of the declaration at compile time and writes the `impl` somebody would otherwise write per field. No reflection reaches the running program — what executes is code a person could have typed ([`docs/02`](../../docs/02-syntax.md) §2.4). It closes [`docs/46`](../../docs/46-standard-library-report.md) §46.16's `@derive` row, and it could not have shipped here until a macro crossed an import |
...
| [`bignum.beck`](bignum.beck) | An integer of any size, as a sign and base-10,000 limbs: schoolbook multiplication, long division, `impl Num`, and every coercion to and from it named rather than implicit. The last floor of the numeric tower ([`docs/46`](../../docs/46-standard-library-report.md)). `bignum"…"` is how one is *written*: an `Int` literal is sixty-four bits, so until that sigil there was no way to put a larger number in a program at all, and every one arrived through `big_of_str` — which raises (`docs/02` §2.5) |
| [`decimal.beck`](decimal.beck) | An exact number of the form `units × 10^-scale`, over `bignum.beck`: canonical so `1.50` and `1.5` are one value, `/` exact or refusing, and three rounding rules rather than one ([`docs/46`](../../docs/46-standard-library-report.md)). Two doors: `decimal_of_str` for text a program was handed, which raises, and `decimal"1.25"` for text a programmer typed, checked while the program compiles (`docs/02` §2.5). One *grammar* behind both — the value is built twice, out of an `Int` and a `Big`, and a differential in the tests holds them together |
...
| [`dates.beck`](dates.beck) | The civil calendar as arithmetic — Hinnant's two functions in Beck, checked against the same two in Rust rather than against themselves — plus `Date`, a `Duration` with its own `impl Num`, clamped month arithmetic, and `YYYY-MM-DD` read and written — twice: `parse_date` for text a program was handed, which raises, and `date"YYYY-MM-DD"` for text a programmer typed, which is checked while the program compiles and costs no effect row (`docs/02` §2.5). One validator behind both |
