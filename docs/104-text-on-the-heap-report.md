...
The second number to read is that **one of the three benchmarks in §104.7 is slower than the
tree-walker, on purpose, and gets slower as it grows**. §104.6 is why, and it is not a defect in
...
## 104.2 A literal is the host's, at a fixed offset
...
What the pool costs is §104.6's third item.

## 104.3 What compiles now that did not
...
## 104.4 What it answers, and what it refuses

| Compiled | How |
|---|---|
| a literal | an offset into §104.2's pool, written into the instruction |
...
## 104.5 One layout, two runtimes — and what writing it twice caught
...
## 104.6 What it costs, said plainly
...
cannot prove that, so `acc + s` allocates the whole accumulator every step. §104.7's `grown` row is
...
rather than smoothed. The third shrinks, and §104.6 is why.
...
**The Cranelift record comparison compared text by its offset**, which is §104.5.
...
## 104.9 What is not built, and the tests that say so

[`101`](101-the-heap-report.md) §101.5's table, less one row:

| Not built | Why it is not a layout problem |
|---|---|
| `list[T]` | the layout is a count and the elements; what is hard is `list_append`'s in-place push, which an arena cannot prove — and §104.6 is the same wall met from the text side |
...
## 104.10 What this corrects, elsewhere
