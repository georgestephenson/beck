...
| 31 | [Generated documentation](31-generated-documentation-report.md) | **Built.** Doc comments in the language (`##`, metadata rather than a form, so documenting a definition is not an API change), `beck doc` for a module, and a language reference derived from the compiler's own tables — the effect/tier matrix is `Tier::discharges` *evaluated*, not transcribed. With the drift gate that fails when the checked-in reference and the compiler disagree, and what it does not do: no cross-module links, no versions, and `missing_docs` deliberately off across 863 public items |
| ref | [The language reference](reference/README.md) | **Generated.** The error index (92 codes), the command reference, the effect and tier matrix, the prelude and the forms — regenerated by `beck doc reference` and gated on every pull request. The design lives above; this is what a program is checked against |
