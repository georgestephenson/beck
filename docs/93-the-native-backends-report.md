...
**985 definitions compile and 144 are refused**, and `examples/todo.beck` compiles **all nine of its
...
that section is about. **All thirty-nine** corpus programs compile their `apply_event` — the step
function of a `durable` fold — and **twenty-six of thirty-nine** compile their `view`
...
share is the heap, which it does not lay out, so it compiles **0 of the corpus's 237 definitions**.
