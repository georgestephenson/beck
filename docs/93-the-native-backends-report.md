...
**985<!--c:native-compiled--> definitions compile and 144<!--c:native-refused--> are refused**, and `examples/todo.beck` compiles **all nine of its
...
function of a `durable` fold — and **26<!--c:corpus-pages--> of 39<!--c:corpus-programs-->** compile their `view`
(`BECK_REQUIRE_LLVM=1 cargo test --release --test native -- --nocapture`; the figures this replaces
were 972, thirty-six and twenty-five of thirty-six, and both the numerator and the corpus have grown
since. Every number in this paragraph is **marked** where it is written and re-derived by
`docs.rs::every_corpus_wide_count_quoted_in_prose_is_the_one_the_tree_has`, which is what stopped
