...
| `pg_catalog`, and therefore `psql`'s `\d` | **Not built**, and the reason has changed: the SQL has joins now, and what is missing is the four catalogue relations to join. `select * from beck_columns` is still the substitute — a table this project invented rather than what any tool expects — and it is a table like any other now, so it can be filtered, grouped and joined, including to itself. The correct long-run answer is a small read-only emulation of `pg_class` and friends |
