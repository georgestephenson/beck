...
| [`12`](12-standards-and-conformance.md) §12.5 | "verified against `psql`, JDBC and BI drivers in CI" — one Rust driver, and no BI driver. `psql` itself is a gate now (`beck-cli/tests/psql.rs`), and JDBC is still untried |
...
| Joins, `group by`, aggregates, `distinct` | **Built, by compiling into the plan** ([`99`](99-the-data-tier-means-of-combination.md) §99.9 item 9). `select … join … on a.k = b.k` becomes the loop `for a in as: for b in bs where b.k == a.k`, which §99.6's recogniser already reads as an equi-join over an index; `group by g` becomes the loop over `list_unique` that `corpus/35-workload.beck` writes by hand, so `count(*)` is the join's tally and no group is built, `min`/`max` are `Op::GroupBy`'s multiset and `sum` its running total; `distinct` is `list_unique`, which is `Op::Distinct`. **Nothing in this surface joins, groups or deduplicates** — `beck-core/src/query.rs` writes an expression and hands it to `Plan::of_query`, which is what "one code path" means here rather than an aspiration. Measured with the recognition switched off beside it: a join over two collections of 200 rows and of 1,600 costs **5,004 backend steps and 40,004** against **244,004 and 15,392,004** for the nested loop an interpreter would have been (`scaling.rs::answering_a_join_in_sql_does_not_reconsider_every_pair`). A query is answered **cold** — its plan is compiled, prepared and thrown away with the answer — which is what keeps the row above true: a read model still costs nothing per event. What `pg_catalog` then added to this surface is the scalar half — expressions in a select list, a `left join`, a comma join, `union` and a scalar subquery that is answered only where it is provably empty. What is still absent: no `right` or `full` join, because the `from` list is joined left-deep; no correlated subquery; no `having`; no join key that is nullable or whose sides are different types; and no expression inside a `group by` or a `distinct`, where it would have to be part of the key the operator builds rather than evaluated over what it produced |
| `count(*)` without scanning | **Built, ungrouped.** `select count(*) from t`, with nothing narrowing it, is answered from the collection's own size: a maintained arrangement is a `BTreeMap` and a `Map` in the accumulator knows its length, and neither was being asked — the query cloned every value and built a `Cell` per column of every one to answer with an integer. `read::Rows::count` is the seam and it defaults to "not without a scan", so a reader that does not implement it is exactly as correct and exactly as slow as it was. Gated by `read_models.rs::a_bare_count_is_answered_without_building_a_row`, whose instrument is a reader that knows the size and **refuses to produce a row** — a query that scanned cannot be answered by it at all. `a_count_that_narrows_anything_still_scans` says where the fast path stops. The *grouped* form is [`99`](99-the-data-tier-means-of-combination.md) §99.4's aggregation half, and this surface reaches it now by compiling into the plan (§99.9 item 9): `select owner, count(*) … group by owner` is answered from the tally a join keeps, so no group is built there either. The ungrouped fast path stays what it is — an arrangement's own size, with no plan compiled at all |
| `pg_catalog`, and therefore `psql`'s `\d` | **Built, as read models** — the small read-only emulation this row asked for. `beck-core/src/pg.rs` derives fourteen relations from the same `Schema` everything else here reads: `pg_class` has a row per read model, `pg_attribute` a row per column, `pg_namespace` two rows, and the nine that describe what a read model has no object of — policies, publications, inheritance, access methods — have their columns and **no rows**, which is what `\d` needs to hear on its way to printing a table. Nothing in `pgwire.rs` knows what a backslash command is: a catalogue query is parsed, planned and scanned by the operators every other query goes through, which is why it cannot drift from the schema ([`adr/0032`](adr/0032-pg-catalog-is-a-read-model.md)). `\d`, `\d <table>`, `\dt`, `\dn` and `\l` answer; a relation that is not here is refused **by name**. What it cost the SQL is the scalar half of one: `case`, a call, a cast, `in`, `is`, a POSIX regular expression, a `left join`, a comma join, `union` and a scalar subquery — the last answered only where dropping its correlated conditions proves it empty, and refused by name otherwise. `select * from beck_columns` is still the table that says what a read model is derived *from*, which is the question `pg_catalog` has no column for |
...
| A BI tool | **Nothing has been tried.** [`12`](12-standards-and-conformance.md) §12.5 claims verification against `psql`, JDBC and BI drivers in CI; there are two clients now — `tokio-postgres` and the `psql` binary — and no BI driver |
