...
a name to agree**, and its refusals are the feature: 316 of the corpus's 325 names rename, nine
decline, and the ninth declines because the edit was made and the file no longer type-checked.
...
The eight are one shape: **a signal whose name is also a model field.** `s.carts` is a field access
...
| **Formatting** | **Not built, and now with a reason rather than a gap.** `beck fmt` exists and wiring it would be four lines — but the lexer *skips* ordinary comments, so a formatted file loses every `#` line in it. **A formatter an editor runs on save must not delete what somebody wrote**; the missing piece is comment-preserving printing, not the wiring |
