...
| `@derive` for JSON | **Built as a macro, and not as a library function** — the distinction is the whole of what is left. `examples/derive.beck` takes a `model`, reads its fields out of the declaration at compile time and emits an `impl ToJson` that names each one ([`02`](02-syntax.md) §2.4, [`102`](102-the-macro-interpreter-report.md)); reflection is still not being added, because a macro reads *syntax* and what runs is code somebody could have typed. Why it is not in `lib/`: a macro does not cross a module boundary — expansion is per module and runs before imports resolve — so the trait and its base impls could ship here and the `derive` could not, and half a facility is not one. [`08`](08-roadmap.md) §8.5.4 carries the crossing |
