...
- **`compiler.yml`** — format, clippy, the full suite (differential and replay harnesses
  ...
  service behind the log-contract test, and a release-profile measurements lane ([ADR-0006](adr/0006-ci-measurements-lane.md)).
...
Owned by the untouched Phase 3 bullet — "`beck init ci`, apko image build in-process, cosign
signing, SBOM" ([`08`](08-roadmap.md)) — plus Phase 4's multi-arch and air-gapped items. In
landing order:
...
2. **The artefact set**: the static binary per target; an apko-built OCI image
   ([`06`](06-kubernetes-and-packaging.md) §6.2); the SBOM apko generates; a cosign signature
   and provenance attestation per artefact; checksums in the release notes. GitHub Releases
   first; §6.7's OCI/ORAS route in Phase 4. The Mere serves packages, not the compiler.
3. **Reproducibility as a gate**: build the image twice, `diff` the digests — §6.2's
   bit-reproducibility claim, mechanised per release.
...
None of this is built. The first slice — tag-triggered signed binaries for one target — should
land with the remaining Phase 3 supply-chain work, because `beck init ci` (below) should emit
for users what we run for ourselves.
...
1. **`beck init ci`** (Phase 3, untouched) emits a workflow mirroring our discipline:
...
| Image size / cold start (§8.3 item 4) | With §28.2's release workflow | Nothing builds an image in CI yet; the budget lands with the artefact |
