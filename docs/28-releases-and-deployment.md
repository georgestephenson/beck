...
- **`compiler.yml`** — format, clippy, the full suite (differential and replay harnesses
  ...
  and verified ([`96`](96-supply-chain-report.md)), and a release-profile measurements lane ([ADR-0006](adr/0006-ci-measurements-lane.md)).
...
[`96`](96-supply-chain-report.md)): there is a command that builds the image, a command that signs
...
2. **The artefact set**: the static binary per target; ~~an apko-built~~ **a `beck`-built** OCI
   image ([`06`](06-kubernetes-and-packaging.md) §6.2, [`96`](96-supply-chain-report.md)); the SBOM
   — which `beck build` writes and which nothing yet **attaches** to the image; a cosign signature
   per artefact, **built** (§96.5), and a provenance attestation, **not**; checksums in the release
...
3. ~~**Reproducibility as a gate**: build the image twice, `diff` the digests~~ — **built, and per
   commit rather than per release** ([`96`](96-supply-chain-report.md) §96.4). Its limit is now
...
`sha256-<digest>.sig` in a registry — is [`96`](96-supply-chain-report.md) §96.7's first row. The
...
1. **`beck init ci`** — **built** ([`96`](96-supply-chain-report.md)) — emits a workflow mirroring
   our discipline:
...
| Image size / cold start (§8.3 item 4) | ~~With §28.2's release workflow~~ — **due now**: CI builds an image ([`96`](96-supply-chain-report.md)) | The reason for the deferral is gone — "nothing builds an image in CI" is no longer true. What the budget still waits on is the *right* binary: the image ships whichever `beck` built it, and until that is the static musl build §28.2 item 1 describes, a size number would be about the wrong artefact (§96.8) |
