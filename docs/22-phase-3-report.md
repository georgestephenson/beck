...
| **A stub cannot *fail*** | **Not built** — [`08`](08-roadmap.md) §8.5.4. A stub supplies the return value of the definition that performs the atom, and a body that raises is refused: the raise is charged to the test block's row and `B0700` says a test performs nothing. So "the peer is down" has no spelling, and the refusal arm a program writes for it is the one arm its tests cannot reach |
