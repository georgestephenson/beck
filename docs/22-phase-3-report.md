...
| …a test block's row must be empty (§21.2's open question) | done — `B0700`, naming the effect, and a `stub` clause is what takes an atom back out of it, because the definitions it stands in for do not run (§22.6) | `beck-core/src/check.rs` |
...
| A `Backend` seam that can install stubs (**not** a §21 item) | added — `Backend::intercepting`, defaulted to "cannot", so a backend that cannot is *skipped* rather than silently run for real. An interception answers with a `Result` and `ExecError` carries the raised value, because `try:` catches by type name and a failure flattened to a message would travel past the handler the program wrote (§22.6) | `beck-core/src/backend.rs` |
...
do. Four of these were closed later and say where, because the code that cites this section cites
it as the place the limit was named.

| | |
|---|---|
| **A stub cannot vary with the call *sequence*** | **Not built.** Rule 3 makes a stub a function of the arguments and nothing more: there is no "fail the first time, succeed the second", because a stub body performs nothing and therefore cannot hold state — it may *fail* now, but the same way every time. Retry logic is testable only through the arguments it passes. Whether that is a limit or the point is a real question — a stateful stub is a small step from a mock framework — and it is recorded as open rather than decided |
| **A stub cannot *fail*** | **Closed**, and it closed a defect beside it. The rule is that a stub stands in for a *definition*: what that definition's row holds is charged to the definition — which the signature already declares — and not to the block that named it. So a `raises(E)` the signature declares is an answer a stub may give (`stub net.out(h): raise Declined`), one it does not declare is `B0708`, and the definitions a stub replaces do not run, so the atom the clause names and the failures those definitions carry stop being the block's — which is what lets an *expectation* call one, and what let a library module have both halves of §21.3 at once. A second peer a performer merely *inherits* is not discharged: it has its own clause to name it, which is what keeps `B0700` naming the host nobody thought about. What a stub still may not do is **perform**: a body reaching an effect is the test's own row and `B0700` as before, because a stub that performed the effect it stands in for would have stood in for nothing. `beck-cli/tests/tests_in_beck.rs` gates all four directions, and [`86`](86-getting-started.md) §86.8 is where the missing arm was found |
