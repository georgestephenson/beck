...
| The macro expander as a fuzzing target | **partly, and less partly than it was.** `Shape::Ui` reaches it and `B0213` shows its ceiling works; `Shape::Sigils` now generates a **user-written** macro and feeds it arbitrary text, which is the half a typed literal adds — a body the lexer takes raw and `node_lit` hands to the interpreter, over an alphabet that includes `$` and `$*`. What is still not generated is a user macro that expands into *more of itself*. What has changed since this row was written is the thing it was waiting on: **F17's macro fuel is built** — `MAX_EXPANSION` bounds what expansion *produces*, `B0214` refuses, and `macro_bomb.rs` is the gate in both directions and for both expanders, so `pending_security.rs` no longer asserts it absent. A generator that writes a doubling macro would now be testing a ceiling rather than finding its absence |
...
| The same treatment for the evaluator | **not built.** This bounds what the *front end* will read; `beck test --fuel` bounds what the evaluator will run ([`53`](53-are-we-fast-yet-report.md)), and nothing generates programs to test that boundary |
