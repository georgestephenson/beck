...
| No memory-unsafety in first-party code | all | structural | `unsafe_code = "forbid"`, inherited by every crate but three. The claim is about the compiler and the runtime; **machine code `beck native` generates is not first-party code in this sense** — it is an artefact, and it runs as a separate process for that reason ([`93`](93-the-native-backends-report.md) §93.1). The three exceptions **deny** rather than forbid, all for one reason: rustc classifies `#[no_mangle]` as unsafe code, and a library nobody can call is not one. `beck-wasm` and `beck-play` are the WebAssembly modules; `beck-prim` is the runtime library a compiled program links ([`93`](93-the-native-backends-report.md) §93.12). Nine `#[allow]`s between them, every one on an export attribute, and no `unsafe` block, `unsafe fn` or raw-pointer read anywhere in any of the three — which for `beck-prim` is a design decision rather than a happy fact, since a runtime library that took a pointer and a length would need an `unsafe` block per primitive. Where the lint could shape the design it did ([`adr/0021`](adr/0021-the-native-backend-writes-ir-and-runs-a-process.md)); where it could not, the extent is a test — `mode_b.rs::an_exported_symbol_is_the_only_exception_to_forbid_unsafe`, which also asserts that every other crate still inherits the workspace lint ([`94`](94-the-client-report.md) §94.6) |
