...
| [0032](0032-the-webassembly-heap-is-the-arena-in-linear-memory.md) | The WebAssembly heap is the same arena in linear memory, and it **grows** rather than being reserved — neither the GC proposal nor refcounting, because a `ref` cannot be read by the host that has to write a state in and read a page back, and both would fork the one layout three backends share |
