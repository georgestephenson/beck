...
| [0031](0031-transcendentals-are-computed-here-and-correctly-rounded.md) | `sin` and `cos` are computed in the runtime library and **correctly rounded**, so the specification is the mathematics rather than a vendored file — IEEE 754 pins neither, and a fold that reached the platform's libm replayed to a different state on a different machine |
| [0032](0032-pg-catalog-is-a-read-model.md) | `pg_catalog` is fourteen read models over the derived schema rather than a case in the wire protocol — a catalogue answered by matching query text is a second description of the schema that no `select` can reach |
| [0033](0033-the-webassembly-heap-is-the-arena-in-linear-memory.md) | The WebAssembly heap is the same arena in linear memory, and it **grows** rather than being reserved — neither the GC proposal nor refcounting, because a `ref` cannot be read by the host that has to write a state in and read a page back, and both would fork the one layout three backends share |
