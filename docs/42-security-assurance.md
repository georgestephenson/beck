...
| Macro expansion fuel | **tested** | F17's expansion half is built: `MAX_EXPANSION` is 100,000 nodes for a module, charged against what expansion *produces* rather than how deep it goes — a doubling macro is shallow, terminates and is enormous — and refused with `B0214`. `macro_bomb.rs` gates it both ways (the tree still compiles; a doubling macro is refused). `pending_security.rs` deleted its absence test when the bound landed, per its own rule. **F17's other half is a typed-literal parser's own work and has nothing to bound yet**, because typed literals wait on the macro interpreter ([`08`](08-roadmap.md) §8.5.4) |
