...
|---|---|---|
| No `unsafe` in first-party code | structural | `unsafe_code = "forbid"` in the workspace manifest, and all nine crates carry `[lints] workspace = true` — inheritance is what makes a workspace lint real, and it is present in every member. The only occurrence of the word in the tree is a comment in `beck-core/src/engine.rs` explaining why a footprint is walked rather than measured |
