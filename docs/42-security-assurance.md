...
|---|---|---|
| No `unsafe` in first-party code | structural | `unsafe_code = "forbid"` in the workspace manifest, and **twelve of the fifteen** crates carry `[lints] workspace = true` — inheritance is what makes a workspace lint real. ~~"all ten crates"~~, three times overtaken: [`93`](93-the-native-backends-report.md) §93.15 corrected it to twelve, then the count reached fourteen with two held to `deny`, and it is now fifteen with three. Those three are `beck-wasm`, `beck-play` and `beck-prim`, and the reason is one: rustc classifies `#[no_mangle]` as unsafe code, and a library nobody can call is not one — a WebAssembly module has to export something, and so does the runtime library a compiled program links ([`93`](93-the-native-backends-report.md) §93.12). Each carries one `#[allow]` per export attribute and **a test** that no other allow site, `unsafe` block or `unsafe fn` exists in it (`mode_b.rs`, `playground.rs`) — an exception whose extent is asserted rather than described. The lint is also why `beck-llvm` exists in the shape it does: a native backend that bound LLVM or ran compiled code in process would have needed `unsafe`, so it writes textual IR and spawns a process instead ([`adr/0021`](adr/0021-the-native-backend-writes-ir-and-runs-a-process.md)), where `beck-clif`'s safe API cost nothing ([`adr/0024`](adr/0024-cranelift-emits-an-object-and-a-linker-makes-it-a-program.md)) |
...
| Macro expansion fuel | **tested** | F17's expansion half is built: `MAX_EXPANSION` is 100,000 nodes for a module, charged against what expansion *produces* rather than how deep it goes — a doubling macro is shallow, terminates and is enormous — and refused with `B0214`. `macro_bomb.rs` gates it both ways (the tree still compiles; a doubling macro is refused) and for **both expanders**, since a `typed macro` is expanded by the checker and draws on a budget of its own ([`102`](102-the-macro-interpreter-report.md) §102.9) — where the failure to guard against is not the charge but the *report*, which a rollback around inference can delete. `pending_security.rs` deleted its absence test when the bound landed, per its own rule. **F17's other half is closed too**: typed literals are built ([`02`](02-syntax.md) §2.5) and a typed literal desugars to a macro call, so the "typed-literal parser's own work" this row waited to bound is a macro body spending the budget above — one surface the finding had named twice. `macro_bomb.rs` gates a sigil whose body does not terminate and one that produces too much |
