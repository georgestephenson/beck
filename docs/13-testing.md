...
| Cranelift **vs** LLVM **vs** reference interpreter | program results | codegen is correct. *All three exist* ([`93`](93-the-native-backends-report.md)): `beck-cli/tests/native.rs` and `tests/cranelift.rs` compare both emitters against the tree-walker over the scalar subset, on the **value and the failure**, and hold the two to accepting and refusing the same definitions. What bounds the differential is the subset rather than the harness: a record, a union and a newtype compile since [`93`](93-the-native-backends-report.md), and text, collections, closures and the effects do not |
...
| **Grammar-aware fuzzing** (`proptest`, per pull request) | Parser, layout algorithm, macro expander, and **typed-literal bodies** — `beck-cli/tests/grammar_fuzz.rs`, eleven shapes, each parameterised so the sizes span every ceiling. `Shape::Sigils` is the one that consumes *arbitrary text* rather than structure: a sigil's body is whatever is between the quotes, taken raw and handed to a macro that reads it, so the alphabet includes the punctuation Beck's own templates are made of. Why: these consume adversarial input (any text file); crashes and hangs here are CVE-shaped. `cargo-fuzz` needs nightly and this workspace pins stable, so the *generator* is the contribution and coverage-guided feedback is owed ([`82`](82-the-edge-report.md) §82.11). |
