...
| **Read models** | **TPC-H**, **ClickBench** | Analytical query latency over a fixed dataset | For the read-model tier once [`05`](05-tier-lowering.md) §5.3 exists — **which is two engines, and this pair splits across them**. TPC-H's 22 joins wait on the view algebra's means of combination ([`99`](99-the-data-tier-means-of-combination.md)); ClickBench scans a fixed dataset no `durable` fold holds in memory, so it waits on §5.3's *analytical* row — the Parquet archive and DataFusion over it — which [`08`](08-roadmap.md) §8.5.4 schedules as a gate in front of this row. **TPC-C is not a fit** and should not be attempted: it assumes update-in-place OLTP, which is not Beck's data model |
