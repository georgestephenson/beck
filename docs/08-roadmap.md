...
compiles. What is left of that bullet is the effects and growing a collection
...
| **Now** | Lane A: `Ord` as a trait, which [`54`](54-ordering.md) writes out and does *not* recommend — so realistically nothing | Lane E: **what is left of the heap** — growing a collection and the effects ([`107`](107-a-map-arrives-read-only-report.md) §107.4, [`109`](109-a-view-arrives-as-a-recipe-report.md) §109.8) — which is what Mode B's codegen and every benchmark number wait on. Note which one is a *decision*: growing a collection is refused rather than unbuilt, so the item behind it is an ownership analysis or a different representation, not a missing emitter | The backend seam exists so these do not interact, and it has now been tested four times. Lane B's render lock is still open and still has no owner — it is a third branch rather than a reason to hold either of these |
...
What is behind the rest of it — growing a collection, the effects — is the same set it
always was, four items shorter: native codegen for anything a program actually manipulates, and Mode B's codegen, which
