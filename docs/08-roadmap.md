...
§110.9) — and growing a collection
...
| **Now** | Lane A: `Ord` as a trait, which [`54`](54-ordering.md) writes out and does *not* recommend — so realistically nothing | Lane E: **what is left of the heap** — growing a collection, and the effects that call *back* into the host ([`107`](107-a-map-arrives-read-only-report.md) §107.4, [`110`](110-a-raise-arrives-report.md) §110.9) — which is what Mode B's codegen and every benchmark number wait on. Note that neither is a missing emitter: growing a collection is a *decision* waiting on an ownership analysis or a different representation, and an effect is a *protocol* with a second direction | The backend seam exists so these do not interact, and it has now been tested five times. Lane B's render lock is still open and still has no owner — it is a third branch rather than a reason to hold either of these |
