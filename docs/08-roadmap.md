...
- ~~LSP: completion, hover with *inferred placement*, go-to-def, rename, inline diagnostics.~~
  ***Built, every entry.*** Hover, go-to-def and inline diagnostics by [`65`](65-lsp-report.md);
  ...
  [`109`](109-the-editor-edits-report.md). Placement is shown twice over: on hover as `@on(tier)`,
...
([`109`](109-the-editor-edits-report.md)); ~~SQL read models, pgwire~~ — **built**
...
| **C — front end and tooling** | `beck-syntax/`, `beck-cli/`, `beck-diag/` | ~~The recursion bound~~ ([`44`](44-wave-0-report.md)); ~~the two syntax decisions~~ ([`10`](10-decisions.md) D21, D22); ~~Unicode and UTS #39~~ ([`44`](44-wave-0-report.md) §44.5); ~~LSP~~ — **built** ([`65`](65-lsp-report.md)) and **finished** ([`109`](109-the-editor-edits-report.md)): references, highlight, rename and inlay hints, so §8.5.2's list of what an LSP is for has no unbuilt entry; ~~`test --update`~~ ([`66`](66-page-snapshots-report.md)); ~~fuzzing~~ ([`85`](85-what-the-generator-found-report.md)). **This lane is empty.** What a *new* Lane C item looks like is in [`109`](109-the-editor-edits-report.md) §109.7: comment-preserving printing, which `textDocument/formatting` waits on, and code actions | A, if a syntax decision changes what the checker sees |
...
| **Any time** | — | Lane F; ~~Lane C's LSP~~ — **built** ([`109`](109-the-editor-edits-report.md)), which empties Lane C; more of SICP | No predecessors, no collisions |
