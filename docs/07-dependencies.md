...
| Release codegen | **LLVM**, as textual IR through the host's `clang` — *not* `inkwell`, which is `unsafe` at the execution engine ([`adr/0021`](adr/0021-the-native-backend-writes-ir-and-runs-a-process.md)); so it is a run-time requirement of `beck native` and **not a crate** | Apache-2.0 with LLVM exception | Best peak code quality available in open source; Cranelift measured ~14% slower, and Perry's 2026 Cranelift→LLVM migration turned a deficit into 1.7×–24.6× wins over Node.js | GCC (GPL + no usable library API); TPDE (promising fast back-end research, too new) |
| Dev codegen / JIT | **Cranelift** — taken as a crate and emitting an **object**, not a JIT: execution in process is `unsafe` ([`adr/0024`](adr/0024-cranelift-emits-an-object-and-a-linker-makes-it-a-program.md)). **Built** ([`93`](93-the-native-backends-report.md)) | Apache-2.0 with LLVM exception | ~40% faster whole-compile and ~10× faster codegen step than LLVM; makes `beck dev` hot reload feel instant | LLVM `-O0` (still far slower); interpreter only (too slow for realistic dev) |
...
| Columnar interchange | **Apache Arrow** | Apache-2.0 | Zero-copy across the wire and into DataFusion; the industry format (§4.4). **Not yet a dependency, and what changed is that there is now something to give it**: a Beck list of `Float` is a dense `f64` run ([`beck-core/src/seq.rs`](../compiler/crates/beck-core/src/seq.rs)), which is a `Float64Array`'s values buffer. The dependency lands with the DataFusion/Parquet row above, because that is where a reader for what it writes exists | bespoke format (no) |
...
| External-tool compatibility | **pgwire** protocol server — **written here, not taken** ([`adr/0020`](adr/0020-the-read-model-speaks-pgwire-by-hand.md)) | — | `psql`/BI/DBeaver see read models as tables (§5.3) — the cheapest trust-builder for adopting teams. "None" in this column meant there is no alternative *protocol*; a server crate carries a type registry, a catalogue and an authentication ladder a read model has nothing to put in, so the subset is ~500 lines here and the gate is somebody else's client | a server crate (`pgwire`, `pgrx`) |
...
| Need | Choice | Licence | Why | Rejected |
|---|---|---|---|---|
| Image build | ~~**apko** (+ **melange** for native deps)~~ — **not taken as a dependency**: `beck image` builds the image itself ([`92`](92-supply-chain-and-release-report.md)), and the apko/melange configs are still emitted as the image's description | Apache-2.0 | apko's *argument* is what this keeps: declarative, no shell execution, therefore **bit-for-bit reproducible**; distroless Wolfi base; daemonless and unprivileged. The reason it did not have to be a dependency is that argument's other consequence — a build that executes nothing has nothing in it a compiler cannot do (§92.1) | **BuildKit** (Apache-2.0; best general builder — keep as `builder = buildkit` escape hatch); Docker build (daemon, root, non-reproducible); Kaniko/Buildah (Dockerfile-shaped, which we don't need); Nix (best-in-class reproducibility, but its learning curve becomes ours); ko/Jib (language-specific) |
| OCI read/write/push | ~~`oci-spec` + `oci-client` crates~~ — **written here** ([`92`](92-supply-chain-and-release-report.md)); what *was* taken is [`flate2`](adr/0025-deflate-so-the-image-build-needs-no-tools.md), because DEFLATE is the one part of the format nobody should write twice | MIT/Apache-2.0 (`flate2`) | The external binary is gone from `beck build`, which is what the row wanted. The format turned out to be four JSON documents and a tar, and the tar had to be written anyway: the image's identity is its digest, so every field that could vary between two runs has to be a constant chosen here, which is a configuration exercise against a library and a hundred lines without one | shelling out permanently; a schema-types crate for four documents |
| Signing / provenance | **Sigstore**, in its **keyed** half, `cosign`-compatible and **written here** over `aws-lc-rs` ([`92`](92-supply-chain-and-release-report.md) §92.6) | Apache-2.0 (the format) | The signature is the simple-signing payload and the artefact layout `cosign verify --key` reads, so consumers need no Beck-specific tool — and `openssl` verifies it in the suite. `sigstore-rs` was not taken: keyless signing needs Fulcio, Rekor and a workload identity this project does not have, so the crate's value is exactly the part that cannot be exercised. **Provenance is not built** | GPG (key management burden); notation (smaller ecosystem); `sigstore-rs` (see left — revisit when there is a transparency log to talk to) |
