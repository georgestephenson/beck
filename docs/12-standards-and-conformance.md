...
| **W3C WebAssembly core + WASI Preview 2 + Component Model** | Mode B client artefacts and the WASM server target; validated with the official spec test suites. *Mode B's kernel is built ([`94`](94-mode-b-report.md)) and it is **core WebAssembly only** — a `wasm32-unknown-unknown` module with four `i32` exports, no WASI and no component model, and nothing in this repository runs the spec suites* |
