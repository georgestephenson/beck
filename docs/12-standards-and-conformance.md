...
| Standard | Status | Where it stands |
|---|---|---|
| **PostgreSQL wire protocol (pgwire)** | **Verified, partly conformed to — and the scope is stated** ([`23`](23-incremental-views-report.md)) | The startup exchange, the simple query and the parameterless extended query in both text and binary format, verified in CI against `tokio-postgres` (`beck-cli/tests/read_models.rs`) — a real driver, not a client written beside the server ([`adr/0020`](adr/0020-the-read-model-speaks-pgwire-by-hand.md)). **The SQL has joins now** — an inner equi-join, a `group by` with `count`/`min`/`max`/`sum`, and `distinct`, none of them interpreted in this surface: they are compiled into the view plan and run by the same operators a program's page uses ([`99`](99-the-data-tier-means-of-combination.md) §99.9 item 9). **And `psql`'s backslash commands work**, for the five that ask about a relation: `\d`, `\d <table>`, `\dt`, `\dn` and `\l`. `pg_catalog` is fourteen read models over the schema the compiler derived ([`adr/0032`](adr/0032-pg-catalog-is-a-read-model.md)), so a catalogue query goes through the same parser, plan and scan as any other and the wire protocol has no branch for one; the gate drives the **`psql` binary itself** (`beck-cli/tests/psql.rs`, skipped without one unless `BECK_REQUIRE_PSQL=1`). What is refused is what a read model has no object of — a function, a role, an index — **by the name of the relation asked for**. JDBC and BI drivers are untried. §23.19 is the row-by-row list |
...
| **OpenMetrics / Prometheus exposition** | **Conformant to the rules, unread by a scraper** | `/_beck/openmetrics` serves OpenMetrics 1.0.0 text beside the dashboard's JSON, off the same table the OTLP export reads, so a metric cannot reach one and not the other. What is held is the specification's own MUSTs — `_total` on a counter, cumulative buckets whose last is `+Inf`, `_count` equal to it, a unit that is a suffix of its family's name, the `# EOF` terminator — and the **Canonical Numbers** rule for an `le`, against the values §"Considerations: Canonical Numbers" publishes. What is *not* held is a foreign reader: nothing in this workspace is a Prometheus scraper, so this is the position [`adr/0030`](adr/0030-the-webassembly-emitter-writes-its-own-bytes.md) takes about a format with no local reader, and `promtool check metrics` over a served endpoint is what would close it |
