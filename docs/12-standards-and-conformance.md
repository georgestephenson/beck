...
| **W3C WebAssembly core + WASI Preview 2 + Component Model** | Mode B client artefacts and the WASM server target; validated with the official spec test suites. *Mode B's kernel is built ([`94`](94-the-client-report.md)) and it is **core WebAssembly only** — a `wasm32-unknown-unknown` module with four `i32` exports, no WASI and no component model, and nothing in this repository runs the spec suites* |
...
| **SLSA v1.2 — target Build L3 *and* Source L3** | provenance attestations for compiler releases — **emitted** ([`92`](92-supply-chain-and-release-report.md)): in-toto over every artefact the release lists, a Sigstore certificate whose identity is the release workflow, and the public transparency log. The **level** is not claimed, because the builder's isolation is GitHub's and this project has not audited it, and a user's `beck build` still attests nothing. Provenance for every user `beck build`; hardened, isolated builders. v1.2's **Source track** is the addition ([`42`](42-security-assurance.md) §42.7): L1 version control → L2 tamper-resistant history with verified contributor identity → L3 enforced, documented protections → L4 no unilateral change to protected branches. Source L3 is mostly a repository-settings exercise and is worth claiming because it is nearly free |
