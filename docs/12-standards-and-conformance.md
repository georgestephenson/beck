...
3. **Stable diagnostic codes** — **Verified, with a named remainder.** The codes are `B0xxx` (the
   model is rustc's `E0xxx`; the prefix is ours): 132 codes in `beck-diag/src/index.rs`, an index
...
| **OpenAPI 3.1 + RFC 9457 problem details** | **Chartered** | Blocked on the `@public(rest)` emitter (Phase 4). The three land as one artefact — the generated surface, its schema derived from types, and `application/problem+json` as its error shape ([`35`](35-standards-landscape.md) §35.5 item 3) — with round-trip tests beside them |
| **gRPC / Protobuf** | **Chartered** | With `@public(grpc)`, Phase 4 at the earliest. Nothing today; `beck-rt/src/telemetry.rs` deliberately avoids tonic/prost |
...
| **CloudEvents 1.0** | **Chartered** | With `ingest(source)`, which is itself unbuilt. Neither has a position yet; if `ingest` is designed without this row, say so here |
...
| **Apache Arrow + Parquet** | **Chartered** | No dependency and no artefact. Conditioned on the data tier's means of combination ([`99`](99-the-data-tier-means-of-combination.md)) and the analytics substrate; official interop test files enter with the first reader or writer |
